...
namespace
{
                                  ...
                                  index_type                                                         j)
  {
    ...
    const scalar_type gr     = model.y()[gammaR.offset];
    const scalar_type gi     = model.y()[gammaI.offset];
  ...
  GridKit::Testing::TestOutcome geometric_inductance_model()
  {
    ...
    success            *= (Lgeo.cols == 2);
    success            *= (Lgeo.size() == 4);
    success            *= close(model.y()[Lgeo.offset], L00_ref);
    success            *= close(model.y()[Lgeo.offset + 1], L01_ref);
    success            *= close(model.y()[Lgeo.offset + 2], L01_ref);
    success            *= close(model.y()[Lgeo.offset + 3], L11_ref);

    model.evaluateResidual();
    for (index_type i = 0; i < Lgeo.size(); ++i)
    {
      success *= close(model.getResidual()[Lgeo.offset + i], 0.0);
    }
    ...
    const scalar_type L01_sagged_ref =
        ...
        / (2.0 * pi);
    success *= close(sagged_model.y()[Lgeo_sagged.offset], L00_sagged_ref);
    success *= close(sagged_model.y()[Lgeo_sagged.offset + 1], L01_sagged_ref);
  ...
  GridKit::Testing::TestOutcome carson_model()
  {
    ...
    success            *= close(model.y()[eta.offset], eta_ref);
    success            *= close(model.y()[gammaR.offset], gamma_r_ref);
    success            *= close(model.y()[gammaI.offset], gamma_i_ref);
    ...
    for (index_type i = 0; i < Rcarson.rows; ++i)
    {
      for (index_type j = 0; j < Rcarson.cols; ++j)
      {
        ...
        success                   *= close(model.y()[Rcarson.offset + i * Rcarson.cols + j], ref.R, 1.0e-12);
        success                   *= close(model.y()[Lcarson.offset + i * Lcarson.cols + j], ref.L, 1.0e-12);
      }
    }

    success *= close(model.y()[Rcarson.offset + 1], model.y()[Rcarson.offset + 2]);
    success *= close(model.y()[Lcarson.offset + 1], model.y()[Lcarson.offset + 2]);

    model.evaluateResidual();
    for (index_type i = 0; i < model.carson().size(); ++i)
    {
      success *= close(model.getResidual()[eta.offset + i], 0.0, 1.0e-10);
    }
  ...
  GridKit::Testing::TestOutcome series_impedance_model()
  {
    ...
    for (index_type i = 0; i < R.rows; ++i)
    {
      for (index_type j = 0; j < R.cols; ++j)
      {
        ...
        const scalar_type Rref = ((i == j) ? model.y()[rskin.offset + i] : 0.0)
                                 + model.y()[Rcarson.offset + k];
        const scalar_type Lref = ((i == j) ? model.y()[lskin.offset + i] : 0.0)
                                 + model.y()[Lgeo.offset + k]
                                 + model.y()[Lcarson.offset + k];
        success *= close(model.y()[R.offset + k], Rref, 1.0e-12);
        success *= close(model.y()[L.offset + k], Lref, 1.0e-12);
      }
    }
    ...
    {
      success *= close(model.getResidual()[series_base + i], 0.0, 1.0e-10);
    }
  ...
  GridKit::Testing::TestOutcome shunt_potential_two_conductor_model()
  {
    ...
    {
      success *= close(model.y()[Gpot.offset + i], 0.0);
    }
    success *= close(model.y()[Gpot.offset + 1], model.y()[Gpot.offset + 2]);
    success *= close(model.y()[Cpot.offset + 1], model.y()[Cpot.offset + 2]);

    model.evaluateResidual();
    for (index_type i = carson_base; i < model.size(); ++i)
    {
      success *= close(model.getResidual()[i], 0.0, 1.0e-8);
    }
  ...
  GridKit::Testing::TestOutcome shunt_potential_three_conductor_passivity_model()
  {
    ...
    {
      return model.y()[signal.offset + i * signal.cols + j];
    };
  ...
  GridKit::Testing::TestOutcome shunt_admittance_model()
  {
    ...
    {
      success *= close(model.y()[G.offset + i], model.y()[Gpot.offset + i], 1.0e-12);
      success *= close(model.y()[C.offset + i], model.y()[Cpot.offset + i], 1.0e-12);
    }
    ...
    {
      success *= close(model.y()[Gext + i], 0.0);
      success *= close(model.y()[Cext + i], 0.0);
    }
    for (index_type i = 0; i < model.shuntAdmittance().size(); ++i)
    {
      success *= close(model.getResidual()[shunt_base + i], 0.0, 1.0e-10);
    }
  ...
  GridKit::Testing::TestOutcome yc_model()
  {
    ...
    {
      success *= close(model.getResidual()[Gc.offset + i], 0.0, 1.0e-8);
    }
    ...
    const std::complex<scalar_type> Z{scalar_model.y()[R1.offset], omega * scalar_model.y()[L1.offset]};
    const std::complex<scalar_type> Y{scalar_model.y()[G1.offset], omega * scalar_model.y()[C1.offset]};
    const auto                      Ycref = std::sqrt(Y / Z);

    success *= close(scalar_model.y()[Gc1.offset], Ycref.real(), 1.0e-8);
    success *= close(scalar_model.y()[Bc1.offset], Ycref.imag(), 1.0e-8);

    scalar_model.evaluateResidual();
    for (index_type i = 0; i < scalar_model.yc().size(); ++i)
    {
      success *= close(scalar_model.getResidual()[Gc1.offset + i], 0.0, 1.0e-8);
    }
  ...
  GridKit::Testing::TestOutcome zc_model()
  {
    ...
    {
      success *= close(model.getResidual()[Rc.offset + i], 0.0, 1.0e-8);
    }
    ...
    success *= close(scalar_model.y()[Rc1.offset], Zcref.real(), 1.0e-8);
    success *= close(scalar_model.y()[Xc1.offset], Zcref.imag(), 1.0e-8);

    scalar_model.evaluateResidual();
    for (index_type i = 0; i < scalar_model.zc().size(); ++i)
    {
      success *= close(scalar_model.getResidual()[Rc1.offset + i], 0.0, 1.0e-8);
    }
  ...
  GridKit::Testing::TestOutcome gamma_model()
  {
    ...
    for (index_type m = 0; m < tvReal.cols; ++m)
    {
      for (index_type n = 0; n < tvReal.cols; ++n)
      {
        ...
        for (index_type i = 0; i < tvReal.rows; ++i)
        {
          ...
          const auto xr       = model.y()[tvReal.offset + r_index];
          const auto xi       = model.y()[tvImag.offset + r_index];
          const auto lr       = model.y()[tiReal.offset + l_index];
          const auto li       = model.y()[tiImag.offset + l_index];
          real               += lr * xr + li * xi;
          imag               += lr * xi - li * xr;
        }
    ...
    {
      success *= close(model.getResidual()[alpha.offset + i], 0.0, 1.0e-8);
    }
    ...
    success *= close(scalar_model.y()[ar.offset], gamma_ref.real(), 1.0e-8);
    success *= close(scalar_model.y()[bi.offset], gamma_ref.imag(), 1.0e-8);
    success *= close(scalar_model.y()[am.offset], gamma_ref.real(), 1.0e-8);
    success *= close(scalar_model.y()[bm.offset], gamma_ref.imag(), 1.0e-8);

    scalar_model.evaluateResidual();
    for (index_type i = 0; i < scalar_model.gamma().size(); ++i)
    {
      success *= close(scalar_model.getResidual()[ar.offset + i], 0.0, 1.0e-8);
    }
  ...
  GridKit::Testing::TestOutcome tau_model()
  {
    ...
    {
      success *= close(model.y()[tau.offset + m],
                       ell * model.y()[betaM.offset + m] / omega,
                       1.0e-10);
    }
    ...
    for (index_type m = 0; m < tau.rows; ++m)
    {
      success *= close(model.getResidual()[tau.offset + m], 0.0, 1.0e-10);
    }
  ...
  GridKit::Testing::TestOutcome h_model()
  {
    ...
    for (index_type m = 0; m < Hr.rows; ++m)
    {
      const scalar_type attenuation =
          std::exp(-model.y()[alphaM.offset + m] * ell);
      const scalar_type phase  = model.y()[betaM.offset + m] * ell;
      const scalar_type c      = std::cos(phase);
      const scalar_type s      = std::sin(phase);
      success                 *= close(model.y()[Hr.offset + m], attenuation * c, 1.0e-10);
      success                 *= close(model.y()[Hi.offset + m], -attenuation * s, 1.0e-10);
    }
    ...
    for (index_type m = 0; m < Hr.rows; ++m)
    {
      ...
      const scalar_type hr = model.y()[Hr.offset + m];
      const scalar_type hi = model.y()[Hi.offset + m];

      model.yp()[alphaM.offset + m] = ad;
      model.yp()[betaM.offset + m]  = bd;
      model.yp()[Hr.offset + m]     = -ell * (ad * hr - bd * hi);
      model.yp()[Hi.offset + m]     = -ell * (ad * hi + bd * hr);
    }

    model.evaluateResidual();
    for (index_type i = 0; i < model.h().size(); ++i)
    {
      success *= close(model.getResidual()[Hr.offset + i], 0.0, 1.0e-10);
    }
  ...
  GridKit::Testing::TestOutcome overhead_gis_path_length()
  {
    ...
    {
      const auto        alphaM  = model.gamma().alphaM();
      const scalar_type atten   = std::exp(-model.y()[alphaM.offset + m] * ell);
      const scalar_type phase   = model.y()[betaM.offset + m] * ell;
      success                  *= close(model.y()[Hr.offset + m], atten * std::cos(phase), 1.0e-10);
      success                  *= close(model.y()[Hi.offset + m], -atten * std::sin(phase), 1.0e-10);
    }
  ...
  GridKit::Testing::TestOutcome overhead_monitor_outputs()
  {
    ...
    const std::string path = "/tmp/gridkit_overhead_monitor_test.csv";
