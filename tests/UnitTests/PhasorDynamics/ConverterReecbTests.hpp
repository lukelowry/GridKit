  ...
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome baseSignals()
      {
        ...
        success                       *= isEqual(reecb.y()[index(Var::IQCMD)], static_cast<ScalarT>(0.05), kTol);
        success                       *= isEqual(reecb.y()[index(Var::IPCMD)], static_cast<ScalarT>(0.25), kTol);
        success                       *= allZero(reecb);
      ...
      TestOutcome zeroTime()
      {
        ...
        success                      *= (reecb.allocate() == 0);
        success                      *= (reecb.verify() == 0);
        reecb.y()[index(Var::IQCMD)]  = static_cast<ScalarT>(0.2);
        reecb.y()[index(Var::IPCMD)]  = static_cast<ScalarT>(0.6);
        success                      *= (reecb.initialize() == 0);
        success                      *= (reecb.tagDifferentiable() == 0);
        success                      *= (reecb.tag()[index(Var::VMEAS)] == true);
        success                      *= (reecb.tag()[index(Var::PMEAS)] == true);

        reecb.yp()[index(Var::VMEAS)]  = static_cast<ScalarT>(1.0);
        reecb.yp()[index(Var::PMEAS)]  = static_cast<ScalarT>(2.0);
        success                       *= (reecb.evaluateResidual() == 0);
        success                       *= isEqual(reecb.getResidual()[index(Var::VMEAS)], static_cast<ScalarT>(-1.0), kTol);
        success                       *= isEqual(reecb.getResidual()[index(Var::PMEAS)], static_cast<ScalarT>(-2.0), kTol);

        reecb.yp()[index(Var::VMEAS)]  = ZERO<RealT>;
        reecb.y()[index(Var::VMEAS)]   = static_cast<ScalarT>(0.99);
        success                       *= (reecb.evaluateResidual() == 0);
        success                       *= isEqual(reecb.getResidual()[index(Var::VMEAS)], static_cast<ScalarT>(10.0), kTol);
      ...
      TestOutcome qPriority()
      {
        ...
        success                      *= (reecb.allocate() == 0);
        success                      *= (reecb.verify() == 0);
        reecb.y()[index(Var::IQCMD)]  = static_cast<ScalarT>(0.3);
        reecb.y()[index(Var::IPCMD)]  = static_cast<ScalarT>(0.9);
        success                      *= (reecb.initialize() == 0);
        success                      *= (reecb.evaluateResidual() == 0);

        const ScalarT ipmax  = std::sqrt(static_cast<ScalarT>(1.1 * 1.1 - 0.3 * 0.3));
        success             *= isEqual(reecb.y()[index(Var::IQMAX)], static_cast<ScalarT>(1.1), kTol);
        success             *= isEqual(reecb.y()[index(Var::IPMAX)], ipmax, kTol);
        success             *= isEqual(reecb.y()[index(Var::IQCMD)], static_cast<ScalarT>(0.3), kTol);
        success             *= isEqual(reecb.y()[index(Var::IPCMD)], static_cast<ScalarT>(0.9), kTol);
        success             *= allZero(reecb);
      ...
      TestOutcome pPriority()
      {
        ...
        success             *= isEqual(reecb.y()[index(Var::IPMAX)], static_cast<ScalarT>(1.1), kTol);
        success             *= isEqual(reecb.y()[index(Var::IQMAX)], iqmax, kTol);
        success             *= isEqual(reecb.y()[index(Var::IQCMD)], static_cast<ScalarT>(0.3), kTol);
        success             *= isEqual(reecb.y()[index(Var::IPCMD)], static_cast<ScalarT>(0.9), kTol);
        success             *= allZero(reecb);
      ...
      TestOutcome voltageBand()
      {
        ...
        success                      *= (reecb.allocate() == 0);
        success                      *= (reecb.verify() == 0);
        reecb.y()[index(Var::IQCMD)]  = ZERO<RealT>;
        reecb.y()[index(Var::IPCMD)]  = static_cast<ScalarT>(0.5);
        success                      *= (reecb.initialize() == 0);
        success                      *= (reecb.evaluateResidual() == 0);

        const ScalarT expected_sdip   = Math::inside(reecb.y()[index(Var::VT)], static_cast<RealT>(0.9), static_cast<RealT>(1.1));
        const ScalarT expected_iqraw  = reecb.y()[index(Var::IQBASE)] + (ONE<RealT> - reecb.y()[index(Var::SDIP)]) * reecb.y()[index(Var::IQV)];
        success                      *= isEqual(reecb.y()[index(Var::SDIP)], expected_sdip, kTol);
        success                      *= isEqual(reecb.y()[index(Var::IQRAW)], expected_iqraw, kTol);
        success                      *= (reecb.y()[index(Var::SDIP)] < static_cast<ScalarT>(0.01));
        success                      *= (reecb.y()[index(Var::IQV)] > static_cast<ScalarT>(0.1));
        success                      *= allZero(reecb);
      ...
      TestOutcome piSaturation()
      {
        ...
        success                      *= (reecb.allocate() == 0);
        success                      *= (reecb.verify() == 0);
        reecb.y()[index(Var::IQCMD)]  = static_cast<ScalarT>(0.15 / 1.13);
        reecb.y()[index(Var::IPCMD)]  = static_cast<ScalarT>(0.5 / 1.13);
        success                      *= (reecb.initialize() == 0);
        success                      *= (reecb.evaluateResidual() == 0);

        const ScalarT piv_arg  = static_cast<ScalarT>(10.0) * reecb.y()[index(Var::EPIV)] + reecb.y()[index(Var::XPIV)];
