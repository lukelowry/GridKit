  ...
  namespace Testing
  {
    ...
    public:
      ...
      TestOutcome hard_coded_residual()
      {
        ...
        // quantities the equations need are evaluated from them. Parameter
        // validation gives Xqp = Xq = 0.18, Xdpp = Xqpp = 0.144, and
        // Xl = 0.1152. Saturation is active at these states.
        const ScalarT expected_psiqpp = -static_cast<ScalarT>(49.) / static_cast<ScalarT>(180.);
        const ScalarT expected_psidpp = static_cast<ScalarT>(79.) / static_cast<ScalarT>(106.);
        ...
        const std::vector<ScalarT> res_answer = {
            ...
            static_cast<ScalarT>(1373332817.) / static_cast<ScalarT>(6993331344.),
            static_cast<ScalarT>(1049213129.) / static_cast<ScalarT>(151090492.)
                + static_cast<ScalarT>(79.) / static_cast<ScalarT>(742.) * expected_ksat,
            -static_cast<ScalarT>(50297.) / static_cast<ScalarT>(5763.),
            -static_cast<ScalarT>(9919368.) / static_cast<ScalarT>(509065.),
            static_cast<ScalarT>(9.) / static_cast<ScalarT>(5.)
                + static_cast<ScalarT>(49.) / static_cast<ScalarT>(4135.) * expected_ksat};
