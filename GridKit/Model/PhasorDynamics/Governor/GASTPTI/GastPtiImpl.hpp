    ...
    namespace Governor
    {
      ...
      void GastPti<scalar_type, index_type>::initModelParams(const ModelDataT& data)
      {
        ...
        auto check_nonnegative = [&](RealT value, const char* name)
        {
          if (value < ZERO<RealT>)
          {
            Log::error() << "GastPti: parameter '" << name << "' must be non-negative\n";
            ++parameter_error_count_;
          }
        };

        check_nonnegative(T1_, "T1");
        check_nonnegative(T2_, "T2");
        check_nonnegative(T3_, "T3");
      ...
      void GastPti<scalar_type, index_type>::initializeMonitor()
      {
        ...
        monitor_->set(Variable::vlv, [this, index]
                      { return y_.getData()[index(GastPtiInternalVariables::VLV)]; });
      }
      ...
      int GastPti<scalar_type, index_type>::initialize()
      {
        ...
        if (static_cast<RealT>(xflow0) < Vmin_ || static_cast<RealT>(xflow0) > Vmax_)
        {
          Log::error() << "GastPti: initial fuel-flow state violates valve limits\n";
          return 1;
        }
          ...
          ScalarT*                        f)
      {
        ...
        f[XVALVE] = -yp[XVALVE] + valve_target / T1_;
        f[XFLOW]  = -yp[XFLOW] + fflow / T2_;
        f[XTEMP]  = -yp[XTEMP] + ftemp / T3_;
