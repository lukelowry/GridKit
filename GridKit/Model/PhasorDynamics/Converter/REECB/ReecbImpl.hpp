  ...
  namespace PhasorDynamics
  {
    namespace Converter
    {
      ...
      int Reecb<scalar_type, index_type>::initialize()
      {
        ...
        y_[VMEAS]     = y_[VT];
        y_[VMEASSAFE] = Math::max(y_[VMEAS], VMEAS_MINIMUM);

        const ScalarT ipcmd0 = toComponentBase(y_[IPCMD]);
        const ScalarT iqcmd0 = toComponentBase(y_[IQCMD]);
        ScalarT       pe0    = ipcmd0 * y_[VMEASSAFE];
        ScalarT       qgen0  = iqcmd0 * y_[VMEASSAFE];
        ...
        y_[PMEAS] = pe0;
        y_[SDIP]  = Math::inside(y_[VT], Vdip_, Vup_);
        y_[VERR]  = Math::deadband2(Vref0_ - y_[VMEAS], dbd1_, dbd2_);
        y_[IQV]   = Math::clamp(kqv_ * y_[VERR], Iql1_, Iqh1_);
        y_[QREF]  = PfFlag_ * y_[PMEAS] * std::tan(pfaref_set_) + pf_off_ * qext0;
        ...
        const ScalarT vpiq_arg = awinit(VFlag_ * y_[VMEAS] + v_off_ * y_[QREF], Kqi_ * y_[EQ], static_cast<ScalarT>(Vmin_), static_cast<ScalarT>(Vmax_));
        ...
        {
          const ScalarT piv_arg = awinit(iqbase_target, Kvi_ * y_[EPIV], -y_[IQMAX], y_[IQMAX]);
        ...
        {
          y_[IQRAW] = QFlag_ * y_[IQBASE] + q_off_ * y_[QV] + (ONE<RealT> - y_[SDIP]) * y_[IQV];
          y_[IQCMD] = toSystemBase(Math::clamp(y_[IQRAW], -y_[IQMAX], y_[IQMAX]));
        };
        ...
        if (Pqflag_ == ZERO<RealT>)
        {
          ...
          const ScalarT iqcmd       = toComponentBase(y_[IQCMD]);
          ...
          y_[IPMAX]  = y_[IPCIRC];
          y_[IPCMD]  = toSystemBase(Math::clamp(ip_star, ZERO<RealT>, y_[IPMAX]));
        }
        else
        {
          y_[IPCIRC] = Imax_;
          ...
          const ScalarT ipcmd       = toComponentBase(y_[IPCMD]);
