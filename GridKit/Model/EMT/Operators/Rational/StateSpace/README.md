...
the same conjugate-pair ordering. Real-valued poles must have real
$\mathbf{C}$ columns and $\mathbf{B}$ rows.
...
$\dot{\mathbf{u}}$ | [-/s] | Input derivative vector | $\dot{\mathbf{u}}\in\mathbb{R}^K$, read from derivative storage
...
For real-valued poles, validation and derived parameters make the
imaginary-memory row zero-coupled.
...
The `out` port exposes the owned algebraic vector $\mathbf{y}$.
