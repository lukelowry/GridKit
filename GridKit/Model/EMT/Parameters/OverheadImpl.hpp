    ...
    namespace Parameters
    {
      namespace Detail
      {
        ...
        public:
          ...
          void setVector(Variable variable, SignalT signal, const std::vector<ScalarT>& values)
          {
            if (!selected(variable))
            {
              return;
      ...
      int Overhead<ScalarT, IdxT>::allocate()
      {
        ...
        y_.assign(static_cast<size_t>(size_), 0.0);
        yp_.assign(static_cast<size_t>(size_), 0.0);
        f_.assign(static_cast<size_t>(size_), 0.0);
        tag_.assign(static_cast<size_t>(size_), false);
        gidx_.resize(static_cast<size_t>(size_));
        std::iota(gidx_.begin(), gidx_.end(), IdxT{0});

        SystemAssembly::bindAll(elements_, y_.data(), yp_.data(), f_.data(), gidx_.data());
      ...
      void Overhead<ScalarT, IdxT>::initializeMonitor()
      {
        ...
        monitor->setMatrix(Variable::R, series_impedance_.R(), y_);
        monitor->setMatrix(Variable::L, series_impedance_.L(), y_);
        monitor->setMatrix(Variable::G, shunt_admittance_.G(), y_);
        monitor->setMatrix(Variable::C, shunt_admittance_.C(), y_);
        monitor->setComplexMatrix(Variable::Tv, gamma_.tvReal(), gamma_.tvImag(), y_);
        monitor->setComplexMatrix(Variable::Ti, gamma_.tiReal(), gamma_.tiImag(), y_);
        monitor->setVector(Variable::Alpha, gamma_.alphaM(), y_);
        monitor->setVector(Variable::Beta, gamma_.betaM(), y_);
        monitor->setVector(Variable::Tau, tau_.tau(), y_);
        monitor->setComplexVector(Variable::H, h_.Hr(), h_.Hi(), y_);
        monitor->setComplexMatrix(Variable::Yc, yc_.Gc(), yc_.Bc(), y_);
        monitor->setComplexMatrix(Variable::Zc, zc_.Rc(), zc_.Xc(), y_);
