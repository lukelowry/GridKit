...
/// Todo - Implement a cleaner way to handle the Sparse versus Dense solvers
                                                 ...
                                                 std::optional<RealT> consistent_tout)
    {
      ...
      if (findConsistent)
      {
        ...
        // The bumped defaults let Newton work harder before quitting;
        // explicit IdaOptions values take precedence.
        IDASetMaxNumItersIC(solver_, options_.max_num_iters_ic.value_or(100));
        IDASetMaxBacksIC(solver_, options_.max_backs_ic.value_or(1000));
        IDASetNonlinConvCoefIC(solver_, options_.nonlin_conv_coef_ic.value_or(1.0e-3));
        IDASetLineSearchOffIC(solver_, options_.line_search_off_ic.value_or(true) ? SUNTRUE : SUNFALSE);
    ...
    int Ida<ScalarT, IdxT>::runSimulation(RealT tf, std::optional<RealT> dt_monitor, const std::optional<OutputCallback> step_callback)
    {
      if (trace_enabled_)
      {
        return runSimulationTraced(tf, dt_monitor.value_or(RealT{0}));
      }
